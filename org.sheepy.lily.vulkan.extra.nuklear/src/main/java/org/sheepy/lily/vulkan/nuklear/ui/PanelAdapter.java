...
public class PanelAdapter extends Notifier<ITextWidgetAdapter.Features> implements IPanelAdapter, ITextWidgetAdapter
{
	...
	public boolean layout(UIContext context)
	{
		...
		backgroundColor.r((byte) panel.backgroundColor().x());
		backgroundColor.g((byte) panel.backgroundColor().y());
		backgroundColor.b((byte) panel.backgroundColor().z());
		backgroundColor.a((byte) panel.backgroundColor().w());
		final var boderColor = context.nkContext.style().window().border_color();
		boderColor.r((byte) panel.borderColor().x());
		boderColor.g((byte) panel.borderColor().y());
		boderColor.b((byte) panel.borderColor().z());
		boderColor.a((byte) panel.borderColor().w());

		final var drawChildren = nk_begin(nkContext, panel.name(), nk_rect(x, y, width, height, rect), style);
		...
		if (drawChildren)
		{
			...
			final var backgroundImage = panel.backgroundImage();
			...
			final var controls = panel.controls();
