																								 ...
																								 IImageWidgetAdapter
{
	...
	public SelectorPanelAdapter(SelectorPanel panel)
	{
		...
		if (panel.printLabels() && panel.vertical() == false)
		{
			throw new AssertionError("Horizontal panel with labels is not supported");
		}

		final var inputProvider = panel.inputProvider();
		fadeTimer = new ProgressTimer(panel.fadeOutMs(), TimeUnit.MILLISECONDS);

		this.panel = panel;
		buttonSize = panel.buttonSizePx();
		resolver = panel.variableResolver();
		...
		final var printLabels = panel.printLabels();
		if (panel.vertical())
		{
	...
	{
		final boolean right = panel.horizontalRelative() == EHorizontalRelative.RIGHT;
		...
		for (final var element : elements)
		{
			...
			tmpDatas.add(new LineData(element, name, panel.name(), image, color, right));
		}
		return tmpDatas;
	}
	...
	public void updateDataLocations(Vector2ic size)
	{
		...
		if (panel.horizontalRelative() == EHorizontalRelative.LEFT)
		{
			x += MARGING_W;
		}
		...
		{
			data.updateRect(x, y, buttonSize + MARGING_W, panel.printLabels());
			if (panel.vertical()) y += buttonSize + MARGING_W;
	...
	public boolean layout(UIContext context)
	{
		...
		for (int i = 0; i < datas.size(); i++)
		{
			...
			if (buttonDrawer.draw(nkContext, isSelected, nkImage, data.color, data.panelButton1Id, data.rectButton))
			{
				...
				if (panel.unsettable() && selectedElement == newSelection)
				{
					newSelection = null;
				}
		...
		if (panel.autoHideLabels() && panel.printLabels())
		{
	...
	{
		return panel.reportingHover() && hovered;
	}
