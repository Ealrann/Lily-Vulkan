...
@AllocationDependency(features = NuklearContext.FeatureIDs.FONT, type = NuklearFontAllocation.class)
public final class NuklearContextAllocation implements IAdapter
{
									 ...
									 @InjectDependency(index = 0) NuklearFontAllocation fontAllocation)
	{
		...
		final var nulkearFontAdapter = nuklearContext.font().adapt(NuklearFontAdapter.class);
		...
		final var vertexBuffer = nuklearContext.vertexBufferMemory();
		final var drawTask = nuklearContext.compositeDrawTask();
	...
	private void layout()
	{
		...
		{
			final var vertexBuffer = nuklearContext.vertexBuffer();
			final var indexBuffer = nuklearContext.indexBuffer();
