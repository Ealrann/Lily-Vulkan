...
public final class LayoutManager
{
						 ...
						 final IObservatoryBuilder observatory)
	{
		...
		this.font = font;
		this.window = window;
		final var ui = (UI) subpass.compositor();
		images = List.copyOf(ui.images());
		process = (GraphicProcess) subpass.lmContainer();
		layoutState = new LayoutState();

		observatory.focus(ui)
				   .explore(UI.FeatureIDs.CURRENTUI_PAGE)
				   .explore(UIPage.FeatureIDs.PANELS)
	...
	private boolean layout()
	{
		...
		final var surface = process.configuration().surface();
