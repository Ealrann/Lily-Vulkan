...
public interface TaskPipeline extends AbstractPipeline, IResourceContainer {
  ...
  interface FeatureIDs {
    ...
    int TASK_PKGS = -1734313282;
  }

  interface Features<T extends Features<T>> extends AbstractPipeline.Features<T>, IResourceContainer.Features<T> {
    ...
    Relation<TaskPkg, List<TaskPkg>, Listener<List<TaskPkg>>, Features<?>> TASK_PKGS = new RelationBuilder<TaskPkg, List<TaskPkg>, Listener<List<TaskPkg>>, Features<?>>().name("taskPkgs").many(true).contains(true).id(TaskPipeline.FeatureIDs.TASK_PKGS).concept(() -> ProcessModelDefinition.Groups.TASK_PKG).build();
