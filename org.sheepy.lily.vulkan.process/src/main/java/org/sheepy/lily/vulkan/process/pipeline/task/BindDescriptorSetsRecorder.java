...
@Allocation
@AllocationDependency(features = BindDescriptorSets.FeatureIDs.DESCRIPTOR_SETS, type = IDescriptorSetAllocation.class)
public final class BindDescriptorSetsRecorder implements IRecordableAdapter
{
									   ...
									   @InjectDependency(index = 0) List<IDescriptorSetAllocation> descriptorSets)
	{
		...
		this.bindPoint = task.bindPoint().value();
	}

	@Override
	public void record(RecordContext context)
	{
		final var descriptorSetsToBind = getDSToBind(context.recordIndex, task.stride());
	...
	private record BindContext(Stream<IDescriptorSetAllocation> descriptorSets, int size)
	{
	}
}
