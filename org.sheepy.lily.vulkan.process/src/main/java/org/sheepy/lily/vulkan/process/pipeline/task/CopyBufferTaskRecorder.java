...
@Allocation
@AllocationChild(allocateBeforeParent = true, features = CopyBufferTask.FeatureIDs.SRC_BUFFER)
@AllocationChild(allocateBeforeParent = true, features = CopyBufferTask.FeatureIDs.DST_BUFFER)
@AllocationDependency(features = CopyBufferTask.FeatureIDs.SRC_BUFFER, type = IBufferReferenceAllocation.class)
@AllocationDependency(features = CopyBufferTask.FeatureIDs.DST_BUFFER, type = IBufferReferenceAllocation.class)
public final class CopyBufferTaskRecorder implements IRecordableAdapter
{
								  ...
								  @InjectDependency(index = 1) IBufferReferenceAllocation dstBufferReference)
	{
		...
		final BufferReference srcBuffer = task.srcBuffer();
		final BufferReference dstBuffer = task.dstBuffer();
		assert srcBuffer.stride() == dstBuffer.stride();
		assert srcBuffer.buffers().size() == dstBuffer.buffers().size();
		final int stride = srcBuffer.stride();
		final int effectiveStride = stride == 0 ? srcBuffer.buffers().size() : stride;
