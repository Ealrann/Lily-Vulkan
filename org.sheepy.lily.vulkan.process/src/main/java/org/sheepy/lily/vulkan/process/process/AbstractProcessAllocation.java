...
public abstract class AbstractProcessAllocation implements IProcessAdapter
{
	...
	{
		final var recorders = process.executionManager().adaptNotNull(IExecutionManagerAdapter.class);
		return recorders.acquireNextPlayer();
	}

	private IFenceView execute(IExecutionPlayer next)
	{
		if (process.waitingFenceDuringAcquire())
		{
			next.waitIdle();
		}
