																						 ...
																						 IExecutionManagerAdapter
{
	...
	{
		final var acquirerAdapter = executionManager.acquirer().adapt(IExecutionAcquirerAdapter.class);
	...
	{
		final int semaphoreCount = executionManager.waitedBy().size();
		final var waitSemaphores = streamWaitData(true).toList();
		final var recorder = getRecorders().get(acquisitionInfo.executionID());
		final var signalSemaphores = signalSemaphores(acquisitionInfo.executionID(), acquisitionInfo.recordIndex());
	...
	protected abstract List<VkSemaphore> signalSemaphores(int executionID, int recordIndex);
}
