...
@Allocation
@AllocationChild(allocateBeforeParent = true, features = BufferBarrier.FeatureIDs.BUFFERS)
@AllocationDependency(features = BufferBarrier.FeatureIDs.BUFFERS, type = IBufferReferenceAllocation.class)
public final class BufferBarrierAllocation implements IBufferBarrierAllocation
{
	...
	{
		srcAccessMask = VulkanModelUtil.getEnumeratedFlag(barrier.srcAccessMask(), EAccess::value);
		dstAccessMask = VulkanModelUtil.getEnumeratedFlag(barrier.dstAccessMask(), EAccess::value);
