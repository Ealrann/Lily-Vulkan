...
public final class ColorAttachmentAllocation implements IExtraAttachmentAllocation
{
									 ...
									 @InjectDependency(index = 0) PhysicalSurfaceAllocation surfaceAllocation)
	{
		...
		final var format = colorAttachment.format().value();
		...
		colorImageView = createAndAllocateImageView(context.getLogicalDevice(), colorAttachment.name());
	...
	private ImageBackend createImage(ProcessContext context, PhysicalSurfaceAllocation surfaceAllocation)
	{
		...
		final int usage = VulkanModelUtil.getEnumeratedFlag(colorAttachment.usages(), EImageUsage::value);
		final var imageBuilder = VkImage.newBuilder(PREFIX + colorAttachment.name(), width, height, colorFormat);
		imageBuilder.usage(usage);
		imageBuilder.initialLayout(colorAttachment.initialLayout());
	...
	{
		return new ClearInfo(false, colorAttachment.clearValue());
	}
