...
public class VkSubpassDescriptionAllocator
{
	...
	public VkSubpassDescription.Buffer allocate(MemoryStack stack, List<Subpass> subpasses)
	{
		...
		else
		{
			...
			colorAttachmentRef.layout(EImageLayout.COLOR_ATTACHMENT_OPTIMAL.value());
	...
	private void fill(MemoryStack stack, List<Subpass> subpasses, final Buffer vkSubpasses)
	{
		for (final var subpass : subpasses)
		{
			...
			final var attachmantRefPkg = subpass.attachmentRefPkg();
			final var refs = attachmantRefPkg != null ? attachmantRefPkg.attachmentRefs() : List.<AttachmentRef>of();

			vkSubpass.pipelineBindPoint(subpass.bindPoint());

				// Color
				final var colorRefs = refs.stream()
										  .filter(a -> a.type() == EAttachmentType.Color)
			...
			fillAttachment(stack, colorRefs, attachmentBuffer -> {
				...
				// Depth
				final var depthRef = refs.stream().filter(a -> a.type() == EAttachmentType.Depth).findFirst();
			depthRef.ifPresent(attachmentRef -> fillAttachment(stack,
				...
				final var inputRefs = refs.stream()
										  .filter(a -> a.type() == EAttachmentType.Input)
										  .collect(Collectors.toList());
	...
	{
		final var attachement = attachmentRef.attachment();
		final int index = attachments.indexOf(attachement);
		vkAttachmentRef.attachment(index);
		vkAttachmentRef.layout(attachmentRef.layout().value());
	}
}
