...
public abstract class AbstractBlitTaskRecorder implements IRecordableAdapter
{
	private static final int FORMAT = EFormat.R8G8B8A8_UNORM.value();
									...
									Vector2ic dstSize)
	{
		...
		if (clearRegions != null)
		{
			...
			final var clearColor = blitTask.clearColor();
	...
	public void record(RecordContext context)
	{
		...
		final var transfertSrc = EImageLayout.TRANSFER_SRC_OPTIMAL.value();
		final var transfertDst = EImageLayout.TRANSFER_DST_OPTIMAL.value();
		final int filter = blitTask.filter().value();
