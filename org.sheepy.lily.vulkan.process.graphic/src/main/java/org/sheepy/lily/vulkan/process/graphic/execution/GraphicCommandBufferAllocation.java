...
@AllocationDependency(parent = GraphicProcess.class, features = {GraphicProcess.FeatureIDs.CONFIGURATION, GraphicConfiguration.FeatureIDs.FRAMEBUFFER_CONFIGURATION}, type = FramebufferAllocation.class)
@AllocationDependency(parent = GraphicProcess.class, features = GraphicProcess.FeatureIDs.SUBPASSES, type = IRecordableAdapter.class)
public final class GraphicCommandBufferAllocation extends AbstractCommandBufferAllocation
{
																	   ...
																	   final FramebufferAllocation framebufferAllocation)
	{
		...
		final int imageID = commandBuffer.imageID();
		final var framebufferPtr = framebufferAllocation.getFramebufferAddresses().get(imageID);
		final var recordInfo = new AbstractProcessCommandBufferHelper.RecordInfo(imageID, commandBuffer.index());
	...
	{
		int res = 0;
		for (final var subpass : process.subpasses())
		{
			final int subpassIndex = subpass.subpassIndex() + 1;
			if (subpassIndex > res)
			{
				res = subpassIndex;
			}
