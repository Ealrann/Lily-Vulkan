...
@AllocationChild(allocateBeforeParent = true, features = RenderPass.FeatureIDs.ATTACHMENTS)
...
@AllocationDependency(parent = GraphicConfiguration.class, features = GraphicConfiguration.FeatureIDs.SWAPCHAIN_CONFIGURATION, type = SwapChainAllocation.class)
@AllocationDependency(features = RenderPass.FeatureIDs.ATTACHMENTS, type = IExtraAttachmentAllocation.class)
public final class RenderPassAllocation implements IRenderPass, IAdapter
{
								 ...
								 @InjectDependency(index = 2) List<IExtraAttachmentAllocation> attachmentAllocations)
	{
		...
		final var process = (GraphicProcess) context.process;
		final var swapConfig = process.configuration().swapchainConfiguration();
													 ...
													 SwapchainConfiguration swapConfig)
	{
		...
		clearInfos.add(new ClearInfo(false, swapConfig.colorAttachment().clearValue()));
