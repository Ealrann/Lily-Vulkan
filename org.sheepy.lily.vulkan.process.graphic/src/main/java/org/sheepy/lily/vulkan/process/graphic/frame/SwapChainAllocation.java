...
@AllocationChild(features = SwapchainConfiguration.FeatureIDs.COLOR_ATTACHMENT)
@AllocationDependency(parent = GraphicConfiguration.class, features = GraphicConfiguration.FeatureIDs.SURFACE, type = PhysicalSurfaceAllocation.class)
public final class SwapChainAllocation implements ISwapChainAllocation, IAdapter
{
							   ...
							   @InjectDependency(index = 0) PhysicalSurfaceAllocation surfaceAllocation)
	{
		...
		final var requiredImageCount = configuration.requiredSwapImageCount();
		...
		final int swapImageUsage = loadSwapChainUsage(configuration);
		final var targetPresentMode = selectPresentMode(context, configuration, surface);
		...
		{
			final boolean accessFromCompute = configuration.allowingAccessFromCompute();
		...
		createInfo.presentMode(targetPresentMode.value());
	...
	{
		final var usages = configuration.swapImageUsages();
		int res = VulkanModelUtil.getEnumeratedFlag(usages, EImageUsage::value);
		if (res == 0)
		{
			res = EImageUsage.COLOR_ATTACHMENT.value();
		}
		return res;
	}
	...
	{
		final String presentationName = presentMode.name();
