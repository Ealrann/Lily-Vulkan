...
public final class VkRenderPassAllocator
{
	...
	public long allocate(MemoryStack stack, GraphicProcess process)
	{
		...
		final var descriptions = getAttachments(process.configuration().swapchainConfiguration());
		...
		renderSubpasses.sort(Comparator.comparingInt(Subpass::subpassIndex));
	...
	private List<Attachment> getAttachments(final SwapchainConfiguration swapchainConfiguration)
	{
		...
		res.add(swapchainConfiguration.colorAttachment());
		res.addAll(renderPass.attachments());
		return res;
	}
}
