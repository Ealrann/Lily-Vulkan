...
@AllocationDependency(features = CompositeImage.FeatureIDs.BACKGROUND, type = IVkImageAllocation.class)
@AllocationDependency(features = {CompositeImage.FeatureIDs.INLAYS, ImageInlay.FeatureIDs.IMAGE}, type = IVkImageAllocation.class)
public final class CompositeImageAllocation implements IVkImageAllocation
{
									 ...
									 @InjectDependency(index = 0) IVkImageAllocation background)
	{
		...
		final var builder = new VkImageBuilder(image.name(),
											   image,
											   vkBackground.width(),
											   vkBackground.height()).initialLayout(null);

		imageBackend = context.executeFunction(builder::build);
		imageView = new VkImageView(vkDevice, image.name(), imageBackend.vkImage(), VK_IMAGE_ASPECT_COLOR_BIT);
	...
	private void assembleImage(IRecordContext recordContext)
	{
		...
		final var initialLayout = image.initialLayout();

		final var srcPtr = vkBackground.ptr();
		...
		vkCmdCopyImage(vkCommandBuffer,
					   srcPtr,
					   EImageLayout.TRANSFER_SRC_OPTIMAL.value(),
					   trgPtr,
					   EImageLayout.TRANSFER_DST_OPTIMAL.value(),
					   region);

		final var blitRegion = VkImageBlit.calloc(1, stack);
		for (var inlay : image.inlays())
		{
	...
	{
		final var imageInlayAdapter = inlay.image().adapt(IVkImageAllocation.class);
		...
		final var scale = inlay.size();
		final var trgSize = new Vector2i(Math.round(vkImageInlay.width() * scale),
										 Math.round(vkImageInlay.height() * scale));
		final var inlayPosition = inlay.position();

		final int trgX = UIUtil.computeXRelative(inlay.horizontalRelative(),
												imageBackend.width(),
												inlayPosition.x(),
												trgSize.x);

		final int trgY = UIUtil.computeYRelative(inlay.verticalRelative(),
												imageBackend.height(),
												inlayPosition.y(),
												trgSize.y);
