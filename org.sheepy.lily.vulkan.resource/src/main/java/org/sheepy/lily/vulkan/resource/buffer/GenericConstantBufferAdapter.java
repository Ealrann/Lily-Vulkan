...
public final class GenericConstantBufferAdapter implements IAdapter
{
	...
	private GenericConstantBufferAdapter(GenericConstantBuffer constantBuffer, IObservatoryBuilder observatory)
	{
		...
		observatory.explore(GenericConstantBuffer.FeatureIDs.VARIABLE_PKG)
				   .explore(ModelVariablePkg.FeatureIDs.VARIABLES, IModelVariable.class)
		...
		observatory.explore(GenericConstantBuffer.FeatureIDs.REFERENCED_VARIABLES, IModelVariable.class)
				   .gather(this::addEntry, this::removeEntry)
				   .adaptNotifier(IModelVariableAdapter.notifierClass())
				   .listenNoParam(() -> valueDirty = true, IModelVariableAdapter.Features.Value);
	}
	...
	{
		final ByteBuffer data = constantBuffer.data();
		if (data != null)
		{
		...
		if (size > 0)
		{
			constantBuffer.data(MemoryUtil.memAlloc(size));
		}
	}

	private void fillValues()
	{
		final var data = constantBuffer.data();
