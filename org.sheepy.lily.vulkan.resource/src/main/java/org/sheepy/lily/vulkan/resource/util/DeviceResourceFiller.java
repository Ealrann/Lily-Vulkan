...
public final class DeviceResourceFiller
{
	...
	private static final class PushBuffersCommand
	{
		...
		public void fillStagingBuffer()
		{
			if (stagingBuffer != null)
			{
				...
				{
					final int size = (int) fillCommand.size();
					fillCommand.fill(MemoryUtil.memByteBuffer(position, size));
					position += size;
				}
	...
	private static final class PushImageCommand
	{
		...
		public void execute(IRecordContext context)
		{
			...
			if (fillCommand.generateMipmap())
			{
				MipmapGenerator.generateMipmaps(context, image, vkImage.initialLayout());
			}
			else
			{
				image.transitionToInitialLayout(context.stack(),
