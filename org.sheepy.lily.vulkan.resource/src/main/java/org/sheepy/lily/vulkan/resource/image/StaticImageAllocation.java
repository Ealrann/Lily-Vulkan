...
public final class StaticImageAllocation implements IMemoryChunkPartAllocation, IVkImageAllocation
{
								...
								final @InjectDependency(index = 0) MemoryChunkAllocation chunkAllocation)
	{
		...
		imageView = new VkImageView(context.getVkDevice(), image.name(), vkImage, VK_IMAGE_ASPECT_COLOR_BIT);
	}
	...
	{
		if (image.fillWithZero())
		{
			MemoryUtil.memSet(trgBuffer, 0);
		}
		else if (image.fillWith() != null)
		{
			MemoryUtil.memCopy(image.fillWith(), trgBuffer);
		}
	}
