...
public final class ImageViewerAllocation implements IMemoryChunkPartAllocation, IVkImageAllocation
{
								...
								final @InjectDependency(index = 0) MemoryChunkAllocation chunkAllocation)
	{
		...
		this.imageView = new VkImageView(context.getVkDevice(), image.name(), vkImage, VK_IMAGE_ASPECT_COLOR_BIT);

		final var dataProviderObservatory = observatory.explore(ImageViewer.FeatureIDs.DATA_PROVIDER)
