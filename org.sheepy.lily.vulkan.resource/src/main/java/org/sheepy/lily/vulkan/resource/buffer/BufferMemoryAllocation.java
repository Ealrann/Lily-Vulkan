...
@AllocationChild(features = BufferMemory.FeatureIDs.BUFFERS)
...
public final class BufferMemoryAllocation implements IMemoryChunkPartAllocation
{
								 ...
								 final @InjectDependency(index = 0) MemoryChunkAllocation chunkAllocation)
	{
		...
		observatory.exploreParent(MemoryChunk.class)
				   .explore(MemoryChunk.FeatureIDs.PARTS)
	...
	{
		return alignmentDataList.get(bufferMemory.buffers().indexOf(buffer));
	}

	@Override
	public Stream<FillCommand> streamFillCommands(final boolean force)
	{
		final var buffers = bufferMemory.buffers();
	...
	{
		final var bufferAllocation = IBufferAllocation.adapt(bufferMemory.buffers().get(index));
