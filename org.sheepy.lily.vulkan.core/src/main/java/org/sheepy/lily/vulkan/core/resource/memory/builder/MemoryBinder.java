...
{
	private final List<AlignmentBuilder.IAlignedResource> alignedResources;
	...
	private Memory.BoundResource bind(final AlignmentBuilder.IAlignedResource alignedResource)
	{
		final var size = alignedResource.size();
		alignedResource.bind(vkDevice, ptr, offset.getAndAdd(size));
		return new Memory.BoundResource(alignedResource.ptr(), size);
	}
}
