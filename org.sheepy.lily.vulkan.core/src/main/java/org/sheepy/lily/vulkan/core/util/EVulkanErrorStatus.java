...
public enum EVulkanErrorStatus
{
	...
	ERROR_INVALID_SHADER_NV(
			NVGLSLShader.VK_ERROR_INVALID_SHADER_NV,
			true,
			"Invalid shaders (NVidia)"),
	...
	EVulkanErrorStatus(final int code, final boolean isError, final String message)
	{
		this.code = code;
	...
	public static EVulkanErrorStatus resolveFromCode(final int errorCode)
	{
		if (errorCode == VK_SUCCESS) return SUCCESS;

		final EVulkanErrorStatus[] values = EVulkanErrorStatus.values();
		for (int i = 0; i < values.length; i++)
		{
			final EVulkanErrorStatus eVulkanError = values[i];
