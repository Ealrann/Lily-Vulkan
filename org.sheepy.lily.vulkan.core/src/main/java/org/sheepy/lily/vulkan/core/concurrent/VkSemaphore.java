...
public final class VkSemaphore implements ISemaphore
{
	...
	public VkSemaphore(VkDevice vkDdevice, String name)
	{
		...
		if (DebugUtil.DEBUG_ENABLED) IVulkanDebugService.INSTANCE.register(semaphorePtr, name);
		semaphoreInfo.free();
	}
	...
	public void free(VkDevice vkDdevice)
	{
		...
		if (DebugUtil.DEBUG_ENABLED) IVulkanDebugService.INSTANCE.remove(semaphorePtr);
	}
}
