...
public final class Window extends Notifier<IWindow.Features> implements IWindow
{
	...
	private void load()
	{
		...
		if (scene.resizeable()) glfwWindowHint(GLFW_RESIZABLE, GLFW_TRUE);
	}
	...
	public void open()
	{
		...
		if (scene.fullscreen())
		{
			...
			windowSize = scene.size();
			scene.size(new Vector2i(mode.width(), mode.height()));
		}
		...
		scene.notifier().listen(fullscreenListener, Scene.FeatureIDs.FULLSCREEN);
		scene.notifier().listen(sizeListener, Scene.FeatureIDs.SIZE);

		Window.this.notify(Features.Open, ptr);
		Window.this.notify(Features.Size, scene.size());
	...
	public void close()
	{
		...
		scene.notifier().sulk(fullscreenListener, Scene.FeatureIDs.FULLSCREEN);
		scene.notifier().sulk(sizeListener, Scene.FeatureIDs.SIZE);
	}
	...
	{
		if (!Objects.equals(notification.newValue(), notification.oldValue()))
		{
	...
	private void resize(Notification notification)
	{
		...
		{
			final var oldSize = (Vector2ic) notification.oldValue();
			final var newSize = (Vector2ic) notification.newValue();
