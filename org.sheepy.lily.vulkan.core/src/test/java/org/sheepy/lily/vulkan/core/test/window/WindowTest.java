...
public class WindowTest
{
	...
	public void testNewWindow()
	{
		...
		LilyLauncher.launch(application, () ->
		{
			...
			Assertions.assertTrue(WindowTestUtil.checkWindowSize(application, expectedWidth, expectedHeight));
	...
	public static final class TestResizeWindowCadenceAdapter implements Runnable
	{
		...
		public void run()
		{
			final boolean expectedSize = WindowTestUtil.checkWindowSize(application, expectedWidth, expectedHeight);
			if (expectedSize)
			{
				expectedWidth = random.nextInt(700) + 201;
				expectedHeight = random.nextInt(700) + 201;

				application.scene().size(new Vector2i(expectedWidth, expectedHeight));
